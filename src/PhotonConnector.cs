namespace Tool.Compet.Photon {
	...
	public class PhotonConnector {
		...
		public readonly Dictionary<int, DkPhotonHub> hubs = new();
		...
		public PhotonConnector(object client, WebSocket socket) {
			...
			var hubList = new List<DkPhotonHub>();

			foreach (var (hubId, hubType) in PhotonServiceRegistry.AppHubServiceTypes()) {
				// Create new hubs for the client.
				var hub = Activator.CreateInstance(hubType, hubId, client, this) as DkPhotonHub;
				if (hub == null) {
